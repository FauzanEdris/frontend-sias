<template>
  <div>
    ...
    <div v-if="listJadwal === null">
      <p>Silahkan Tambahkan Jadwal Atau Semester Baru.</p>
    </div>
    <div v-else>
      <b-navbar>
        <b-navbar-nav>
      ...
      >
        ...
        <template v-slot:cell(aksi)="data">
          {{ data.item.id }}
          <b-button v-b-modal.modalEdit @click="editJadwal(data.item)" variant="outline-warning" size="sm" title="Edit User">
            Edit
          </b-button>
          <b-button @click="hapus(data.item._id)" variant="outline-danger" size="sm" title="Delete User">
            Delete
          </b-button>
        </template>
      </b-table>
...
<script>
import formEdit from '~/components/operator/formEditJadwal'
export default {
  components: {
    formEdit
  },
  data () {
    return {
      id: this.$store.state.operator.listJadwal.id,
      listJadwal: this.$store.state.operator.listJadwal.list,
      rows: this.$store.state.operator.totalJadwal,
      perPage: 5,
      currentPage: 1,
      thead: [
        {
  ...
  async fetch ({ store, params }) {
    await store.dispatch('operator/GET_JADWAL')
    await store.dispatch('operator/GET_ASDOS')
  },
  methods: {
    async setData () {
      await this.$store.dispatch('operator/GET_JADWAL')
      this.id = this.$store.state.operator.listJadwal.id
      this.listJadwal = this.$store.state.operator.listJadwal.list
      this.rows = this.$store.state.operator.totalJadwal
    },
    editJadwal (data) {
      this.$store.commit('operator/EDIT_JADWAL', data)
    },
    download () {
      this.$store.dispatch('operator/DOWNLOAD_EXCEL')
    },
    hapus (data) {
      this.$bvModal.msgBoxConfirm('Apakah Anda Yakin Ingin Menghapus Data?', {
        ...
        if (value === true) {
          data = this.id + ' ' + data
          // console.log(data)
          await this.$store.dispatch('operator/HAPUS_JADWAL', data)
          this.setData()
        }
      })
    }
